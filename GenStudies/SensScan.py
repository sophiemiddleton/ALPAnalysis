...
def main():
    ...
    for i, mass in enumerate(masses):
        ...
        while coup < end_coupling:
            ...
            if(0.6*N_prod_proc[mass][0]*xsec_factor*z_in_hcal/z_all)> 2.44:#(0.88/22)
...
def main():
    ...
    file_path = "/Users/sophie/LDMX/software/NewClone/ALPs/displaced/m20_prima.lhe"
    valid_z = []
    with open(file_path, 'r') as lhe_file:
        for line in lhe_file:
            if line.startswith("#vertex"):  # Look for vertex lines
    ...
    while coup < end_coupling:
        for i, mass in enumerate(masses):
            ...
            #mass_factor=(mass/20)**4
            gfactor = (1e-3 / coup)**2
            xsec_factor = (coup / 1e-3)**2
            z_optimal = (valid_z * gfactor * mass_factor)
            z_in_hcal = 0
            z_all = 0
            for i, z in enumerate(z_optimal):
                z_all +=1
                if z > 800 and z < 5700:
                    z_in_hcal += 1
            #print(mass,coup,N_prod_proc[mass][0]*xsec_factor*z_in_hcal/z_all)
            if(N_prod_proc[20][0]*xsec_factor*z_in_hcal/z_all)> 2.44:
                coups.append(coup)
                masses_i.append(mass)
            coup +=5e-7


    ax2.scatter(masses_i, coups)
    plt.xscale('log')
    plt.yscale('log')
    plt.ylabel('$g_{a \gamma} [ GeV^{-1}]$')
    plt.xlabel('mass [MeV]')
