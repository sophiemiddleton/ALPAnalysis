...
class WabEvent:
    ...
    def loop(self, event_type):
        ...
        nentries = 10000#self.tin.GetEntriesFast();
        ...
        nElecECAL=0.
        layers=[]
        for i in range(10000):#nentries):
            self.tin.GetEntry(i);
            hasElecInECal = False
            for p, part in enumerate(self.simParticles):
                if part.second.getPdgID() == 11 and part.second.getProcessType() == 13:
                    if (abs(part.second.getEndPoint()[0])<470 and abs(part.second.getEndPoint()[1]) < 350 and part.second.getEndPoint()[2]< 750):
                        hasElecInECal = True
                        nElecECAL +=1
                        print(part.second.getEndPoint()[0],part.second.getEndPoint()[1],part.second.getEndPoint()[2])
            ...
            if(hasSide == True):
                nSomethingSide+=1
            if(hasSide == True and hasBack == True and len(self.ecalHits) == 0):
                nSideAndBack +=1
            if(hasSide == True and hasBack == True and len(self.ecalHits) != 0):
                nECALSideBack +=1
            if(hasSide == False and hasBack == True and len(self.ecalHits) != 0):
                nECALBack +=1
            if(hasSide == False and hasBack == True and len(self.ecalHits) == 0):
                nOnlyBack +=1
            if(hasSide == True and hasBack == False and len(self.ecalHits) != 0):
                nECALSide +=1
            if(hasSide == True and hasBack == False and len(self.ecalHits) == 0):
                nOnlyside +=1
            if(hasSide == False and hasBack==False and len(self.ecalHits) == 0):
                nNothing += 1
            # ECAL SP :
            """
        ...
        fig, ax = plt.subplots()
        print(layers)
        ntrue, bins, patches = ax.hist(layers, bins=25,histtype='step',range=(0,25),color='red',label="wab")
        ...
        plt.show()
        print(nECAL,nHCAL)
        print("has Elec in ECAL ", 100*((nElecECAL/(nentries))),"%")
        print("has ECAL ", 100*((nECAL/(nentries))),"%")
        ...
        print("has back + side ", 100*nSideAndBack/nentries, "%")
        print("has ECAL + back + side ", 100*nECALSideBack/nentries, "%")
        print("has ECAL + side ", 100*nECALSide/nentries, "%")
        print("has ECAL + back ", 100*nECALBack/nentries, "%")
        ...
        print("has nothing ", 100*nNothing/nentries, "%")
        print("has something in side",100*nSomethingSide/nentries,"%")
